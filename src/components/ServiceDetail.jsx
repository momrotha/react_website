      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row -mx-4">
          <div className="md:w-1/2 px-4">
            <img className="w-full h-[400px] object-cover rounded-lg" src={product.image} alt={product.title} />
          </div>
          <div className="md:w-1/2 px-4">
            ...
            <div className="mt-2 text-gray-700 dark:text-gray-300 text-sm flex items-center">
              <span className="font-bold">Rating:</span>
              <span className="text-yellow-400 ml-2">
                {"★".repeat(Math.round(product.rating?.rate || 0))}
                {"☆".repeat(5 - Math.round(product.rating?.rate || 0))}
              </span>
              <span className="ml-2">({product.rating?.count || 0} reviews)</span>
            </div>
